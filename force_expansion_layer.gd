...
func _process(delta: float) -> void:
	...
	for unit: Dictionary in units:
		...
		unit["altitude_phase"] = float(unit.get("altitude_phase", 0.0)) + delta
...
func _update_authority_unit(unit: Dictionary, delta: float) -> void:
	...
	if order == "unloading":
		return
	if str(unit.get("kind", "")) == "medic" and (order == "idle" or order == "hold"):
		if _medic_step(unit, delta):
			return
	if str(unit.get("kind", "")) == "engineer" and (order == "idle" or order == "hold"):
		if _engineer_step(unit, delta):
			return
	unit["action_state"] = _state_for_order(order)
	super._update_authority_unit(unit, delta)

func _state_for_order(order: String) -> String:
	if order == "move" or order == "attack_move" or order == "patrol":
		return "moving"
	if order == "construct":
		return "constructing"
	if order == "repair":
...
func _medic_step(unit: Dictionary, delta: float) -> bool:
	...
	unit["support_clock"] = float(unit.get("support_clock", 0.0)) + delta
	if float(unit.get("support_clock", 0.0)) >= 0.30:
		unit["support_clock"] = 0.0
		_spawn_effect("spark", unit_position.lerp(patient_position, 0.55), Color("7dffd0"), 0.30)
	return true
...
func _engineer_step(unit: Dictionary, delta: float) -> bool:
	...
	unit["support_clock"] = float(unit.get("support_clock", 0.0)) + delta
	if float(unit.get("support_clock", 0.0)) >= 0.32:
		unit["support_clock"] = 0.0
		_spawn_effect("spark", unit_position.lerp(building_position, 0.58), Color("ffd66e"), 0.30)
	return true
...
func _complete_transport_deployment(transport_id: int) -> void:
	...
	for index in range(payload.size()):
		var cargo_kind: String = str(payload[index])
		var angle: float = TAU * float(index) / float(maxi(1, payload.size())) + 0.35
		var spawn_position: Vector2 = position + Vector2.from_angle(angle) * 52.0
		var passenger: Dictionary = _spawn_unit(cargo_kind, AUTHORITY, spawn_position)
		passenger["arrival_flash"] = 0.75
		_spawn_effect("construct", spawn_position, passenger.get("accent", Color.WHITE) as Color, 0.46)
	transport["payload"] = []
	transport["order"] = "idle"
	transport["action_state"] = "idle"
	_spawn_effect("construct", position, Color("eaffff"), 0.72)
...
func _move_unit(unit: Dictionary, target: Vector2, delta: float) -> void:
	...
	var horizontal_open: bool = not _blocked_by_map(horizontal_slide, clearance) and not _blocked_by_buildings(horizontal_slide, clearance)
	var vertical_open: bool = not _blocked_by_map(vertical_slide, clearance) and not _blocked_by_buildings(vertical_slide, clearance)
...
func _revealed(world_point: Vector2) -> bool:
	for entity: Dictionary in units:
		if str(entity.get("team", "")) != AUTHORITY:
			continue
		var reveal_range: float = 360.0
		if str(entity.get("kind", "")) == "recon":
			reveal_range = 610.0
		elif bool(entity.get("airborne", false)):
			reveal_range = 500.0
		if (entity.get("pos", Vector2.ZERO) as Vector2).distance_to(world_point) < reveal_range:
			return true
	for building: Dictionary in buildings:
		if str(building.get("team", "")) == AUTHORITY and (building.get("pos", Vector2.ZERO) as Vector2).distance_to(world_point) < 330.0:
			return true
	return false
...
func _draw_force_overlays() -> void:
	for unit: Dictionary in units:
		...
		if kind in INFANTRY_KINDS:
			_draw_infantry_overlay(unit, position, facing, right, accent)
		elif kind in VEHICLE_KINDS:
			_draw_vehicle_overlay(unit, position, facing, right, accent)
		elif kind in AIR_KINDS:
			_draw_air_overlay(unit, position, facing, right, accent)
		_draw_action_animation(unit, position, facing, accent)

func _draw_infantry_overlay(unit: Dictionary, position: Vector2, facing: Vector2, right: Vector2, accent: Color) -> void:
	var kind: String = str(unit.get("kind", ""))
	var pulse: float = sin(float(unit.get("walk_phase", 0.0)) * 0.7) * 1.5
	if kind == "breacher":
		draw_arc(position, 23.0 + pulse, 0.0, TAU, 16, accent, 2.5)
		draw_line(position + right * -10.0, position + facing * 24.0 + right * -10.0, Color("fff1de"), 5.0)
	elif kind == "ranger":
		draw_circle(position, 19.0, Color(accent.r, accent.g, accent.b, 0.32))
		draw_line(position + right * -4.0, position + facing * 39.0 + right * -4.0, Color("effaff"), 2.2)
	elif kind == "medic":
		draw_arc(position, 23.0 + pulse, 0.0, TAU, 18, Color("7dffd0"), 2.2)
		draw_line(position + Vector2(-9.0, 0.0), position + Vector2(9.0, 0.0), Color("effff8"), 2.2)
		draw_line(position + Vector2(0.0, -9.0), position + Vector2(0.0, 9.0), Color("effff8"), 2.2)
	elif kind == "engineer":
		draw_rect(Rect2(position - Vector2(13.0, 13.0), Vector2(26.0, 26.0)), Color(accent.r, accent.g, accent.b, 0.28), false, 2.0)
		draw_line(position + right * 8.0, position + facing * 22.0 + right * 8.0, Color("ffe9aa"), 3.0)
	elif kind == "recon":
		draw_arc(position, 26.0 + pulse, -0.85, 0.85, 16, Color("8deaff"), 2.0)
		draw_circle(position + facing * 11.0, 4.0, Color("ecffff"))
	elif kind == "warden":
		draw_arc(position, 30.0 + pulse, 0.0, TAU, 20, Color("d7b6ff"), 3.0)
		draw_line(position - facing * 6.0 - right * 14.0, position - facing * 6.0 + right * 14.0, Color("f7eaff"), 4.0)

func _draw_vehicle_overlay(unit: Dictionary, position: Vector2, facing: Vector2, right: Vector2, accent: Color) -> void:
	var kind: String = str(unit.get("kind", ""))
	var tread: float = sin(float(unit.get("walk_phase", 0.0)) * 1.3) * 2.0
	if kind == "mech_mover":
		draw_circle(position, 29.0, Color("3d3c4b"))
		draw_arc(position, 30.0, 0.0, TAU, 20, accent, 2.5)
		draw_line(position - right * 15.0, position - right * 24.0 + Vector2(0.0, tread), Color("e9dfbd"), 6.0)
		draw_line(position + right * 15.0, position + right * 24.0 + Vector2(0.0, -tread), Color("e9dfbd"), 6.0)
		draw_line(position + facing * 5.0, position + facing * 44.0, Color("fff0c4"), 9.0)
		return
	var half_length: float = 29.0 if kind != "pursuit_skimmer" else 24.0
	var half_width: float = 18.0 if kind != "bastion_tank" else 24.0
	draw_line(position - facing * half_length - right * half_width, position + facing * half_length - right * half_width, Color("161c29"), 11.0)
	draw_line(position - facing * half_length + right * half_width, position + facing * half_length + right * half_width, Color("161c29"), 11.0)
	draw_line(position - facing * half_length, position + facing * half_length, accent, 18.0)
	draw_circle(position - right * half_width, 5.0 + tread * 0.2, Color("0d1420"))
	draw_circle(position + right * half_width, 5.0 - tread * 0.2, Color("0d1420"))
	if kind == "siege_crawler":
		draw_line(position + facing * 6.0, position + facing * 52.0, Color("fff1c6"), 10.0)
		draw_circle(position + facing * 53.0, 7.0, Color("fff4db"))
	elif kind == "arc_lancer":
		draw_arc(position + facing * 12.0, 18.0, -1.1, 1.1, 14, Color("dcfbff"), 4.0)
	elif kind == "pursuit_skimmer":
		draw_line(position - facing * 26.0, position - facing * 42.0, Color("aef3ff"), 4.0)
		_draw_hover_orbs(position, right, accent)
	elif kind == "bastion_tank":
		draw_circle(position, 18.0, Color("754950"))
		draw_line(position + facing * 7.0, position + facing * 43.0, Color("fff0df"), 8.0)
	elif kind == "troop_carrier":
		draw_rect(Rect2(position - Vector2(24.0, 17.0), Vector2(48.0, 34.0)), Color("31564f"), true)
		draw_rect(Rect2(position - Vector2(24.0, 17.0), Vector2(48.0, 34.0)), accent, false, 2.0)
		var payload: Array = unit.get("payload", []) as Array
		draw_string(font, position + Vector2(-20.0, 4.0), "SQUAD %d" % payload.size(), HORIZONTAL_ALIGNMENT_LEFT, 50, 10, Color("effff8"))

func _draw_hover_orbs(position: Vector2, right: Vector2, accent: Color) -> void:
	draw_circle(position - right * 17.0 + Vector2(0.0, 5.0), 6.0, Color(accent.r, accent.g, accent.b, 0.38))
	draw_circle(position + right * 17.0 + Vector2(0.0, 5.0), 6.0, Color(accent.r, accent.g, accent.b, 0.38))

func _draw_air_overlay(unit: Dictionary, position: Vector2, facing: Vector2, right: Vector2, accent: Color) -> void:
	var altitude: float = 11.0 + sin(float(unit.get("altitude_phase", 0.0)) * 3.0) * 3.0
	var draw_position: Vector2 = position + Vector2(0.0, -altitude)
	var kind: String = str(unit.get("kind", ""))
	draw_circle(position + Vector2(7.0, 9.0), 18.0, Color(0.02, 0.05, 0.10, 0.36))
	draw_line(draw_position - right * 26.0, draw_position + right * 26.0, Color(accent.r, accent.g, accent.b, 0.78), 8.0)
	draw_line(draw_position - facing * 18.0, draw_position + facing * 26.0, Color("e9faff"), 6.0)
	if kind == "sky_lifter":
		draw_circle(draw_position - right * 20.0, 8.0, Color("a2efff"))
		draw_circle(draw_position + right * 20.0, 8.0, Color("a2efff"))
		var payload: Array = unit.get("payload", []) as Array
		draw_string(font, draw_position + Vector2(-22.0, 5.0), "DROP %d" % payload.size(), HORIZONTAL_ALIGNMENT_LEFT, 56, 10, Color("eaffff"))
	elif kind == "specter_flyer":
		draw_arc(draw_position, 28.0, 0.0, TAU, 20, Color(accent.r, accent.g, accent.b, 0.38), 1.8)
		draw_line(draw_position - facing * 30.0, draw_position - facing * 47.0, Color("c9baff"), 2.0)
	elif kind == "lunar_bomber":
		draw_circle(draw_position + facing * 12.0, 11.0, Color("ffbf82"))
		draw_line(draw_position + right * 17.0, draw_position + facing * 14.0 + right * 17.0, Color("fff0d8"), 4.0)

func _draw_action_animation(unit: Dictionary, position: Vector2, facing: Vector2, accent: Color) -> void:
	var flash: float = float(unit.get("action_flash", 0.0))
	if flash > 0.0:
		var muzzle: Vector2 = position + facing * (float(unit.get("radius", 18.0)) + 10.0)
		draw_circle(muzzle, 8.0 + flash * 12.0, Color(accent.r, accent.g, accent.b, 0.44))
		draw_circle(muzzle, 3.0 + flash * 5.0, Color("fff8df"))
	var state: String = str(unit.get("action_state", "idle"))
	if state == "healing" or state == "repairing" or state == "constructing":
		var ring: float = 20.0 + sin(mission_clock * 8.0 + float(unit.get("id", 0))) * 3.0
		draw_arc(position, ring, 0.0, TAU, 20, Color(accent.r, accent.g, accent.b, 0.72), 1.8)
	elif state == "deploying" or state == "unloading":
		draw_arc(position, 32.0 + sin(mission_clock * 7.0) * 4.0, 0.0, TAU, 20, Color("eaffff"), 2.2)
